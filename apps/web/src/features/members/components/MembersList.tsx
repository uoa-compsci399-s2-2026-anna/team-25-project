...
import { getMembers, MEMBERS_PAGE_SIZE } from "../members.queries"
import {
...
export const MembersList = async ({ searchParams }: { searchParams: Promise<SearchParams> }) => {
  ...
  const { docs: members, totalPages } = await getMembers(toMemberFilters(params), {
  ...
  if (members.length === 0) {
    return (
      <div className="flex flex-col items-center gap-2 p-10 text-center text-muted-foreground md:p-12">
        {/* totalDocs and totalPages both come back 0 past the end of a relationship-filtered
            query, so the page number is what tells these two states apart. */}
        {params.page > 1 ? (
          <>
            <p>This page is past the end of the results.</p>
            <Link
              className="underline underline-offset-2"
              href={getPageHref(Math.max(totalPages, 1))}
            >
              Go to the last page
            </Link>
          </>
